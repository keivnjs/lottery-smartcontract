...
contract('Election', function (accounts) {
  ...
  it('initializes candidates with correct values', function () {
    Election.deployed()
      ...
      .then(function (candidate) {
        assert.equal(candidate[0], 'Neymar', 'candidate has correct name');
        assert.equal(candidate[1], 0, 'candidate has correct vote count');
        return instance.candidates(1);
      })
      .then(function (candidate) {
        assert.equal(candidate[0], 'Messi', 'candidate has correct name');
        assert.equal(candidate[1], 0, 'candidate has correct vote count');
      });
  });
  ...
  it('throws exception for invalid candidates', function () {
    Election.deployed()
      ...
      .then(function (candidate) {
        assert.equal(candidate[2], 1, 'Neymar did not receive any votes');
        return instance.candidates(2);
      })
      .then(function (candidate) {
        assert.equal(candidate[2], 0, 'Messi did not receive any votes');
